...
#
# CROSSED COMBINATIONS: 6  (3 diagnosis × 2 LGBTQ+)
#
# MARGINAL GROUPS (added alongside the 6 crossed cells):
#   diagnosis margins:  aut_tot | altra_diagnosi_tot | no_diagnosi_tot
#                       (regardless of LGBTQ+ status)
#   LGBTQ+ margins:     lgbtq_tot | no_lgbtq_tot
...
#
# pct = count / totale_var * 100  (% over respondents to that variable; sums to 100%)
#
# EXPORTED FUNCTIONS:
...
DIAGNOSI_FILTERS <- list(
  ...
  # altra_diagnosi: NO autism (aut == 4 or NA),
  # but at least one other diagnostic column is != 4 and not NA
  altra_diagnosi = function(data) {
    # All diagnostic columns except autism
    diag_cols <- c("pers", "ansia", "umore", "comp_al", "appr",
                   "psicosi", "ocd", "ptsd", "dipendenza", "altro")

    # TRUE if at least one non-autism diagnostic column has a valid non-4 value
    ha_altra <- Reduce(`|`, lapply(diag_cols, function(col) {
      is_valid(data[[col]]) & data[[col]] != 4  # value != 4 means diagnosed
    }))

    # Restrict to respondents who definitely have NO autism (aut == 4, not NA)
    no_aut <- is_valid(data$aut) & data$aut == 4

    ha_altra & no_aut  # both conditions must hold simultaneously
  },
