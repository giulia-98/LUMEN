...
# -----------------------------------
# The priority variables are measured on a 0–10 ratio scale. They show strong
# negative skew (high means, long lower tails) and some crossed subgroups have
# n < 30. Non-parametric tests do not assume normality or homoscedasticity and
# are therefore preferred for hypothesis testing.
# The direction of effects is described using the arithmetic mean (rather than
# the median), which is more informative for ratio-scale data.
#
...
# Uses the arithmetic mean (appropriate for ratio-scale 0–10 data) to describe
# the direction and magnitude of the difference.
# Returns a list with:
#   sym   — ">" if group 1 mean > group 2 mean, "<" otherwise, "=" if equal
#   med1  — rounded mean for group 1  (named med1 for backward compatibility)
#   med2  — rounded mean for group 2  (named med2 for backward compatibility)
# ------------------------------------------------------------------------------
direzione_effetto <- function(col, grp1_nome, grp2_nome) {
  ...
  if (grp1_nome %in% c("lgbtq", "no_lgbtq")) {
    ...
    med1 <- mean(base[[col]][m1_mask], na.rm = TRUE)  # mean for group 1 (ratio scale)
    med2 <- mean(base[[col]][m2_mask], na.rm = TRUE)  # mean for group 2 (ratio scale)
    sym  <- if (med1 > med2) ">" else if (med1 < med2) "<" else "="
    return(list(sym = sym, med1 = round(med1, 2), med2 = round(med2, 2)))
  }

  med1 <- mean(data[[col]][m1_mask], na.rm = TRUE)  # mean for group 1 (ratio scale)
  med2 <- mean(data[[col]][m2_mask], na.rm = TRUE)  # mean for group 2 (ratio scale)
...
genera_testo <- function(col) {
    ...
    if (!is.na(kw_fdr) && kw_fdr < 0.05) {
      ...
      for (cp in coppie_diag) {
        ...
        if (!is.na(p_fdr) && p_fdr < 0.05) {
          ...
          frasi <- c(frasi, sprintf(
            "  • %s (m=%.2f) %s %s (m=%.2f)  [W=%s, r=%s, p_fdr=%s]",
  ...
  if (!is.na(lgbt_fdr) && lgbt_fdr < 0.05) {
    ...
    frasi <- c(frasi, sprintf(
      "Effetto LGBT+ significativo: LGBT+ (m=%.2f) %s No LGBT+ (m=%.2f)  [W=%s, r=%s, p_fdr=%s].",
