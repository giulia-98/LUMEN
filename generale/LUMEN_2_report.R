...
# Diagnosis variables for which value 4 ("No diagnosi") is excluded from bar plots
DIAG_VARS <- c("aut", "adhd", "pers", "ansia", "umore", "comp_al",
...
make_bar_plot <- function(var_name) {
  ...
  # For diagnosis variables (aut … altro), exclude value 4 ("No diagnosi") from the plot
  if (var_name %in% DIAG_VARS) {
    plot_df <- plot_df[plot_df$valore != "4", ]  # valore is character in df_combo
  }
